...
** Packages
   ...
   Let's start by loading ~package.el~ and add melpa archive.
...
** Completions
  ...
  Sometimes there are annotations that Emacs should display when
  selecting from a list of candidates (see for instance
  ~describe-function~).
...
* Coding settings
  ...
  #+begin_src emacs-lisp :tangle init.el
    ...
    (use-package highlight-indent-guides :hook prog-mode
      :custom
...
** LSP
  ...
  #+begin_src emacs-lisp :tangle init.el
    (use-package eglot :commands (eglot-ensure
                                  ...
                                  eglot format
                                  eglot-format-buffer)
      :hook ((eglot-managed-mode . (lambda () (jsb/eglot-format-on-save-maybe))))
    ...
    (defvar jsb/no-format-major-modes '(tsx-ts-mode typescript-ts-mode python-ts-mode c-ts-mode c-mode)
      "Major modes that eglot should not auto-format.")

    (defun jsb/eglot-format-on-save-maybe ()
      "Formats buffer with eglot unless major-mode matches an ignored major-mode."
      (unless (memq major-mode jsb/no-format-major-modes)
        (add-hook 'before-save-hook #'eglot-format-buffer 0 t)))
  #+end_src

  We use =C-c ! l= or =C-x C-h .= to show the relevant diagnostics, we
  don't want flymake to show diagnostics on the end of the line.
  ...
  #+begin_src emacs-lisp :tangle init.el
    ...
    (use-package flycheck :after lsp-mode
      ...
      :init (setq flycheck-auto-display-errors-after-checking nil))
  #+end_src
  ...
  We want Emacs to always use =tree-sitter= to parse whatever source
  files we're editing.

  #+begin_src emacs-lisp :tangle init.el
    (use-package treesit-auto :demand
  ...
  For repositories which includes projects whose roots are not in the
  repository root we need to tell =project= how to find
  these. Typically we can find the root by looking up from a current
  file to a directory satisfying a predicate.

  #+begin_src emacs-lisp :tangle init.el
    (require 'project)

    (defun find-inner-project (project-mode predicate)
      "Define how to find an inner project for PROJECT-MODE, using PREDICATE."
      (unless (symbolp project-mode)
        (error "project-mode should be a symbol"))
      (unless (functionp predicate)
        (error "predicate should be a function"))
      ;; only run the predicate for a given mode
      (add-hook 'project-find-functions
    	    (lambda (dir)
    	      (when (eq major-mode project-mode)
    		(when-let ((root (funcall predicate dir)))
    		  (cons 'inner-project root))))))

    (cl-defmethod project-root ((project (head inner-project)))
      (cdr project))
  #+end_src

  Eglot should be run for C code.
...
** Formatting
   ...
   #+begin_src emacs-lisp :tangle init.el
     ...
     (defvar prog-mode-formatters '((c-mode . clang-format-buffer)
                                    ...
                                    (go-mode . gofmt)
                                    (go-ts-mode . gofmt)
                                    (haskell-mode . ormolu-format-buffer)
                                    (rust-mode . rust-format-buffer)
                                    (rust-ts-mode . rust-format-buffer)
                                    (typescript-ts-mode . prettier-prettify)
                                    (haskell-mode . ormolu)
...
** Version control
   ...
   I'm also experimenting with jujutsu, for which I also want emacs support:

   #+begin_src emacs-lisp :tangle init.el
     ;; Install jj support for emacs builtin VC
     (use-package vc-jj :ensure t)

     ;; Install description mode for jj
     (use-package jjdescription :ensure t)
   #+end_src
...
** Golang
  ...
  #+begin_src emacs-lisp :tangle init.el
    ;; a Go module is a directory containing a go.mod file
    (find-inner-project 'go-mode (lambda (dir) (locate-dominating-file dir "go.mod")))
  #+end_src
  ...
  #+begin_src emacs-lisp :tangle init.el
    (use-package go-ts-mode :mode "\\.go\\'"
      :bind (:map go-ts-mode-map (("C-c C-a" . go-import-add)
                                  ("C-c C-k" . godoc)
  ...
  #+begin_src emacs-lisp :tangle init.el
    (use-package flycheck-golangci-lint :after go-ts-mode
  ...
  #+begin_src emacs-lisp :tangle init.el
    (use-package gotest :after go-ts-mode
  ...
  #+begin_src emacs-lisp :tangle init.el
    (use-package go-dlv :defer t :after go-ts-mode :commands (dlv-current-func))
  #+end_src

** TypeScript and Web stuff

   First we need to tell Emacs and treesitter how to install
   treesitter grammars for =tsx=.

   #+begin_src emacs-lisp :tangle init.el
     ;; let treesit know how to install grammars for tsx
     (add-to-list
      'treesit-language-source-alist
      '(tsx . ("https://github.com/tree-sitter/tree-sitter-typescript" "master" "tsx/src")))

     (defun local/install-tsx-grammar-if-needed ()
       (unless (treesit-language-available-p 'tsx)
         (treesit-install-language-grammar 'tsx)))

     (add-hook 'tsx-ts-mode-hook #'local/install-tsx-grammar-if-needed)
   #+end_src

   Then we want =eglot= to run and start =typescript-language-server=.
   ...
   #+begin_src emacs-lisp :tangle init.el
     ...
     (add-to-list 'prog-mode-formatters '(tsx-ts-mode . prettier-prettify))
   #+end_src
...
** Haskell
   ...
   I want to be able to open a git repository where a cabal file is
   not necessarily in the root and have eglot figure out the project
   by itself.

   #+begin_src emacs-lisp :tangle init.el
     (defun find-cabal-file (dir)
       "Returns non-nil if DIR contains a *.cabal file."
       (consp (directory-files dir nil ".*\.cabal")))

     (find-inner-project 'haskell-mode (lambda (dir) (locate-dominating-file dir #'find-cabal-file)))
   #+end_src
   ...
   #+begin_src emacs-lisp :tangle init.el
     ...
     ;; Don't show the errors from FlyCheck when I hover over an error
     ;; Stolen from: https://github.com/flycheck/flycheck/issues/1332
...
** Python
   ...
   For Python we want it to figure out the project-specific paths of
   all executables, for that we use =pet=.

   #+begin_src emacs-lisp :tangle init.el
     (use-package pet :commands (pet-mode)
       :init
       (setq pet-find-file-functions '(pet-find-file-from-project-root)
     	pet-venv-dir-names '(".venv" "venv" ".env" "env"))
       (add-hook 'python-base-mode-hook 'pet-mode -10))
   #+end_src

   We also want to have black automatically format our files, for
   which we can use =blacken=.

   #+begin_src emacs-lisp :tangle init.el
     (use-package blacken :commands (blacken-mode) :hook (python-base-mode python-mode))
     (add-hook 'python-base-mode-hook 'lsp-deferred)
   #+end_src

** DBC

   We have vendored a GitHub hosted at [[https://github.com/alexmurray/dbc-mode][alexmurray/dbc-mode]].

   #+begin_src emacs-lisp :tangle init.el
     ;; add vendor directory to load-path
     (add-to-list 'load-path (concat "/home/" user-login-name "/dotfiles/emacs/vendor"))
...
** CI
   ...
   I would like to be able to lint the configuration files of my CI
   and since I mainly use GitHub Actions, that can be linted by
   actionlint, which can be integrated in flymake.

   #+begin_src emacs-lisp :tangle init.el
     (use-package flymake-actionlint
       :hook ((yaml-mode yaml-ts-mode) . flymake-actionlint-action-load-when-actions-file))
   #+end_src
...
** Visual appearance
   ...
   We also want the system to automatically change the theme depending
   on the system's color scheme, i.e. switch to the dark theme if the
   rest of the system is using a dark theme (uses xdg-desktop-portal).

   #+begin_src emacs-lisp :tangle init.el
     (use-package auto-dark :init (auto-dark-mode) :defer nil
       :config (setq auto-dark-themes '((modus-vivendi) (modus-operandi))))
   #+end_src
...
** Fonts
   ...
   #+begin_src emacs-lisp :tangle init.el
     ...
     (defvar font-preferences '("iosevka term" "Hasklig" "Inconsolata Nerd Font Mono 16"
     ...
     (add-hook 'server-after-make-frame-hook
           (lambda () (let ((fonts (cl-remove-if-not 'font-existsp font-preferences)))
     		    (unless (or (not fonts) (eq window-system nil))
                          (progn
...
* Org configuration
 ...
 When looking at clock tables and other time durations, the number of
 hours is clearer to read than it aggregated into number of days.
...
* Eshell
  ...
  I found =Eshell smart display= on the [[https://masteringemacs.org/article/complete-guide-mastering-eshell][Eshell article on Mastering
  ...
  Some commands expect a visual terminal (in contrast to the
  line-based support provided by Eshell), we need to let Eshell know
  that these need special handling.

  #+begin_src emacs-lisp :tangle init.el
    (require 'em-term)
    (add-to-list 'eshell-visual-commands "jj")
  #+end_src

* Terminal emulator

  vterm is a fast terminal emulator running inside Emacs.

  #+begin_src emacs-lisp :tangle init.el
    (use-package vterm :ensure t :commands (vterm-mode)
      ;; automatically set vterm name to *vterm PROJECT* inside PROJECT.
      :hook ((vterm-mode . (lambda () (jsb/vterm-rename-buffer-project-local))))
      :bind (:map vterm-mode-map
    	      ("C-q" . vterm-send-next-key)))

    (defun jsb/vterm-rename-buffer-project-local ()
      "Renames current buffer to *vterm PROJECT* (or *vterm PROJECT<COUNTER>*
    if there are already multiple terminals in current project.)"
      (if (project-current)
          (let* ((base-name (concat "*vterm " (project-name (project-current)) "*"))
    	     (buffer-name (generate-new-buffer-name base-name)))
    	(rename-buffer buffer-name))))
  #+end_src

  We want to be able to access the =server-name= variable to reuse the
  Emacs instance for terminal activities requiring an editor.

  #+begin_src emacs-lisp :tangle init.el
    (defun jsb/expose-server-name-envvar ()
      "Exposes SERVER-NAME variable behind EMACS_SERVER_NAME environment variable."
      (if (boundp 'server-name)
          (setenv "EMACS_SERVER_NAME" server-name)
        (setenv "EMACS_SERVER_NAME")))

    (advice-add 'server-start :after #'jsb/expose-server-name-envvar)

    ;; We also want to make sure that the EDITOR environment variable
    ;; points to the spawning Emacs instance.
...
** GPTel
   ...
   I want to be able to query a local LLM running in ollama.

   #+begin_src emacs-lisp :tangle init.el
     (use-package gptel
       :init (setq gptel-model 'mistral:latest
...
** Agentic workflows
   ...
   Claude code is quite useful and it'd be even more useful if it was
   available at a couple of key strokes.

   #+begin_src emacs-lisp :tangle init.el
     (use-package claude-code :ensure t :vc (:url "https://github.com/stevemolitor/claude-code.el" :rev :newest)
       :config
       ...
       ;; auto-accept/plan/confirm modes after invoking
       ;; claude-code-cycle-mode / C-c M.
       :bind (:repeat-map my-claude-code-map ("M" . claude-code-cycle-mode)))

     (use-package monet :vc (:url "https://github.com/stevemolitor/monet" :rev :newest) :after claude-code)
   #+end_src

   It seems my claude-code expects the buffer to be named in a certain
   way, and my vterm configuration breaks that, instead I will try out
   =eat= backend.

   #+begin_src emacs-lisp :tangle init.el
     (use-package eat :ensure t :hook (eshell-load . eat-eshell-mode))
   #+end_src
