...
* Coding settings
  ...
  #+begin_src emacs-lisp :tangle init.el
    ...
    (use-package yasnippet :after eglot :hook (eglot-mode . yas-minor-mode) :ensure t)
  #+end_src
...
** TypeScript and Web stuff
   ...
   #+begin_src emacs-lisp :tangle init.el
     ...
     (use-package tide :ensure t :after typescript-mode
       :hook ((typescript-mode . tide-setup)
              (typescript-mode . tide-hl-identifier-mode))
       :config
       (setq company-tooltip-align-annotations t))

     (add-to-list 'prog-mode-formatters '(typescript-mode . prettier-prettify))
   #+end_src

   TSX files should be treated as TypeScript files.
   ...
   #+begin_src emacs-lisp :tangle init.el
     (use-package web-mode
       ...
       :bind (:map web-mode-map ("C-c C-RET" . company-complete))
       :config (flycheck-add-mode 'typescript-tslint 'web-mode)
       :ensure t)
   #+end_src

   =prettier= should automatically format my web files.
...
* Key stroke reminders
 ...
 #+begin_src emacs-lisp :tangle init.el
   (use-package which-key :delight t :ensure t
     :init (which-key-mode)
...
** Tree-sitter
   ...
   Tree-sitter is a new parsing system with Emacs bindings, it was
   presented on StrangeLoop [[https://www.thestrangeloop.com/2018/tree-sitter---a-new-parsing-system-for-programming-tools.html][here]].

   #+begin_src emacs-lisp :tangle init.el
     ;; Install tree-sitter dynamic modules and languages in a writable directory.
     (let ((vars-and-directories-alist
            `((tsc-dyn-dir . ,(expand-file-name "tree-sitter/tsc-dyn" user-emacs-directory))
              (tree-sitter-langs-grammar-dir . ,(expand-file-name "tree-sitter/langs" user-emacs-directory))
              (tree-sitter-langs--dir . ,(expand-file-name "tree-sitter/langs" user-emacs-directory)))))
       (dolist (el vars-and-directories-alist)
         (let ((var (car el))
               (dir (cdr el)))
           (make-directory dir t)
           (set var dir))))

     ;; Enable tree-sitter for all programming derived modes.
     (use-package tree-sitter :ensure t :delight
       :init (add-hook 'tree-sitter-after-on-hook 'tree-sitter-hl-mode)
       :config
       (require 'tree-sitter-langs)
       (global-tree-sitter-mode))

     ;; Install the language bundle.
