...
** Golang
  ...
  #+begin_src emacs-lisp :tangle init.el
    (use-package go-mode :mode "\\.go\\'" :after eglot
      ...
      :hook (go-mode . (lambda () (add-hook 'before-save-hook #'eglot-format-buffer -10 t)))
      :config (add-to-list
  ...
  #+begin_src emacs-lisp :tangle init.el
    (use-package flycheck-golangci-lint :straight t :after go-mode
      :hook ((go-mode . flycheck-golangci-lint-setup)
             ...
             (go-mode . local/setup-sage-golangci-lint)))

    (defun local/setup-sage-golangci-lint ()
      "Instructs flycheck-golangci-lint to use the shipped binary and
        config from a sage directory, if present."
      (let* ((sage-dir (concat (vc-root-dir) ".sage/"))
             (golangci-lint-config (concat sage-dir "tools/golangci-lint/.golangci.yml"))
             (golangci-lint-exe (concat sage-dir "bin/golangci-lint")))
        (when (file-exists-p golangci-lint-exe)
          (setq-local flycheck-golangci-lint-executable golangci-lint-exe))
        (when (file-exists-p golangci-lint-config)
          (setq-local flycheck-golangci-lint-config golangci-lint-config))))
  #+end_src
...
* Visual appearance
  ...
  #+begin_src emacs-lisp :tangle init.el
    (defun jsb/correct-symbol-bounds (pretty-alist)
    ...
    (defun jsb/ligature-list (ligatures codepoint-start)
    ...
    (setq jsb/hasklig-ligatures
          (let* ((ligs '("&&" "***" "*>" "\\\\" "||" "|>" "::"
                         ...
                         "<<" "<<<" "<+>" ".." "..." "++" "+++"
                         "/=" ":::" ">=>" "->>" "<=>" "<=<" "<->")))
            (jsb/correct-symbol-bounds (jsb/ligature-list ligs #Xe100))))

    ;; nice glyphs for haskell with hasklig
    (defun jsb/set-hasklig-ligatures ()
      "Add hasklig ligatures for use with prettify-symbols-mode."
      (interactive)
      (setq prettify-symbols-alist
            (append jsb/hasklig-ligatures prettify-symbols-alist))
